 ...
 * @data 03/25/2019
 */
void ping_read(float *distance, int *overflow, float *pulse_time, float *pulse_ms) {

    distance_ptr = distance;
    overflow_ptr = overflow;
    pulse_time_ptr = pulse_time;
    pulse_ms_ptr = pulse_ms;
    //Send starting pulse
    ping_send();
    ...
    *pulse_time = falling_edge - rising_edge;
    ...
    if (rising_edge > falling_edge) {
        *overflow_ptr = *overflow_ptr + 1;
        *pulse_time_ptr = *pulse_time_ptr + 0xFFFFFF;
    }
    //lcd_printf("%.2f\ntime:%.2f",diff,*pulse_time);
    *pulse_ms_ptr = (*pulse_time_ptr / 16000.0);
    //Calculate the distance
    *distance_ptr = ((*pulse_time_ptr / 16000000.0) * 34300.0)/2;
